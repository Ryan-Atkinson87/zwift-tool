...
allowed-tools: Read, Glob, Grep, Edit, Write, Bash(git log:*), Bash(git diff:*), Bash(find *:*), Bash(ls *:*), Bash(cd *:*), Bash(wc *:*), Bash(npm run *:*), Bash(mvn *:*), Bash(gh issue *:*)
---
...
Update `internal_docs/CURRENT_OPEN_ISSUES.md` to reflect the current state of open GitHub issues.

1. Run `gh issue list --state open --limit 100 --json number,title,body,labels,milestone` to get all open issues
2. Read `internal_docs/CURRENT_OPEN_ISSUES.md` if it exists
3. Rewrite the file with the current open issues, listed by number ascending
4. Any issues that were previously listed but are no longer open will be removed automatically by the rewrite
5. If there are no open issues, write the file with the header and a note that there are none
...
> Auto-generated by `/review-check-docs`. Do not edit manually.
...
Reproduce each issue's full description (body) as-is, preserving any markdown formatting, acceptance criteria, and checklists from the GitHub issue. Separate each issue with a horizontal rule.

Print a summary of what changed (issues added, issues removed) compared to the previous version of the file.